...
import { rawMenuConfig } from "../Sidebar/menuLoader";
...
const buildPathMap = () => {
  ...
  const walk = (items) => {
    for (const it of items || []) {
      if (it?.path && it?.key) map[it.path] = { code: String(it.key), label: it.label || String(it.key) };
      if (it?.children) walk(it.children);
    }
  };
  walk(rawMenuConfig?.sidebar1?.items);
  walk(rawMenuConfig?.sidebar2?.items);
  return map;
};
