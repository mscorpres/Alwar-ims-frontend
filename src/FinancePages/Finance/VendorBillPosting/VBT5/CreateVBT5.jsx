...
export default function CreateVBT5({ editingVBT, setEditingVBT, setVBTData }) {
  ...
  const submitFunction = async () => {
    ...
    setLoading(true);
    const response = await imsAxios.post("/tally/vbt05/add_vbt05", {
      ...finalObj,
  ...
  const getGl = async () => {
    const response = await imsAxios.get("/tally/vbt05/vbt05_gl_options");
    let arr = [];
    if (data.length > 0) {
      arr = data.map((d) => {
        return {
    ...
    if (editingVBT?.length > 0) {
      ...
      let arr = editingVBT?.map((row) => {
        ...
        return {
          ...
          in_po_rate: +Number(row.in_po_rate).toFixed(3),
          freight: 0,
          bill_qty: row.qty,
          ...
          vendorAmount:
            row.in_gst_type == "L"
              ? (
                  ...
                  Number(Number(row.in_gst_sgst).toFixed(3))
                ).toFixed(3)
