...
import {
  getPurchaseGlCodeValue,
  getPurchaseGlOptions,
  isVbt01StyleModule,
  mergeGlSelectOptions,
  resolveGlOptionsList,
  VBT01_API_PREFIX,
...
function VBT01Report({
  ...
  const getVBTDetail = async (minIdArr) => {
    ...
    const payload =
     isVbt01StyleModule(tallyPrefix)
        ? {
    ...
    if (response.success) {
      ...
      await getGl(getPurchaseGlOptions(data), type);
