...
export default function ComponentImages({
  ...
  const handleFetchImages = async (key: string) => {
  ...
  const handleDeleteImage = async (image: string) => {
    if (showImages) {
      if (typeof showImages.productKey === "string") {
        const response = await executeFun(
          () => deleteImage(showImages.productKey, image),
          `delete${image}`
        );
      }
    }
  ...
  useEffect(() => {
    if (showImages && showImages.productKey) {
      handleFetchImages(showImages.productKey);
    }
  }, [showImages]);
  return (
    <Drawer
      title={`${showImages?.name ?? ""}`}
      placement="right"
      width="40vw"
      onClose={() => setShowImages(false)}
      open={showImages ? true : false}
    >
              ...
              <Col span={12}>
                ...
                >
                  ...
                  <Popconfirm
                    ...
                    cancelText="No"
                    okButtonProps={{
                      loading: loading(`delete${image.id}`),
                    }}
                    // load={loading(`delete${image.id}`)}
                ...
                <Space style={{ paddingLeft: 10 }} direction="vertical">
                  <Typography.Text>{image.uploadedBy}</Typography.Text>
                  <Typography.Text>{image.uploadedBy}</Typography.Text>
                </Space>
              </Col>
            ))}
